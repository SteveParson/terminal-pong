...
int main(int argc, char **argv)
{
	...
	int x = 11, y = 11, maxX = 0, maxY = 0, nextX = 0,
	    nextY = 0, dirX = 1, dirY = 1, padX = 10, padY = 10;
	double relposbX = 0, relposbY = 0, relpospY = 0;
	char kbinput;
	...
	while (true) {
		...
		if (WINSIZE_CHANGE) {
			...
			if (nextX == 10 && dirX == -1 && padY <= nextY &&
			    padY + PADDLE_SIZE >= nextY)
				dirX *= -1;
			if (nextX == 9 && dirX == 1 && padY <= nextY &&
			    padY + PADDLE_SIZE >= nextY)
				dirX *= -1;
